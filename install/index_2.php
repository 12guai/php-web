...
if (isset($_POST['save'])) {
	...
	if (!@$link = mysqli_connect($_POST['db_host'],$_POST['db_username'],$_POST['db_password'])) {
		...
		$pwd = md5($_POST[db_pass]);
		$sql = "update mkcms_manager set m_name='".$_POST['db_user']."' , m_password='".$pwd."' where m_id = 1";
   ...
   <form method="post"> 
    <div class="container wrap1" style="height:680px;">
            ...
            <div class="col-sm-8 col-md-8 center-auto pd-sm-50 pd-xs-20 main_content">
                ...
                <p class="text-left font16">数据库服务器  <span class="input-note">按服务器提供商提供的填写，一般为 localhost 或者 127.0.0.1</span></p>
                    <div class="form-group mg-t10">
                        <input id="db_host" type="text" name="db_host" class="input" placeholder="请输入数据库服务器" value="localhost"/>
                    </div>
				<p class="text-left font16">数据库名  <span class="input-note">按服务器提供商提供的填写，如果有权限也可自己创建。</span></p>
                    <div class="form-group mg-t10">
                        <input id="db_name" name="db_name" type="text" class="input" placeholder="请输入数据库名" />
                    </div>
					
					<p class="text-left font16">数据库用户名  <span class="input-note">按服务器提供商提供的填写</span></p>
                    <div class="form-group mg-t10">
                        <input id="db_username" name="db_username" name="db_password" type="text" class="input" placeholder="请输入数据库用户名" />
                    </div>
				
					<p class="text-left font16">数据库密码  <span class="input-note">按服务器提供商提供的填写</span></p>
                    <div class="form-group mg-t10">
                        <input id="db_password" name="db_password" type="text" class="input" placeholder="请输入数据库密码" />
                    </div>
					<p class="text-left font16">管理员帐号  <span class="input-note">设置后台管理员帐号</span></p>
                    <div class="form-group mg-t10">
                        <input id="db_user" name="db_user" type="text" class="input" placeholder="管理员帐号" />
                    </div>
					<p class="text-left font16">管理员密码  <span class="input-note">设置后台管理员帐号</span></p>
                    <div class="form-group mg-t10">
