...
if ($i<1){echo 'active';}
echo '"><a class="stui-banner__pic" href="seacher-'.$listm['data'][$i]['link'].'.html" alt="'.$listm['data'][$i]['link'].'" style="background: url('.$listm['data'][$i]['cover'].') rgba(98,71,136,0) no-repeat; background-position: center center; background-size: cover; display: block; width: 100%; height: 100%;" title="'.$listm['data'][$i]['link'].'"><div class="am-slider-desc"><div class="am-slider-content"><h3 class="am-slider-title">'.$listm['data'][$i]['link'].' - '.$listm['data'][$i]['desc'].'</h3><p></p></div></div></a></div>';}?>
</div>
</div>
<style type="text/css">
			...
			<ul class="stui-vodlist clearfix">
				<?php
					for($i=0;$i<$pagecount;$i++){
						$ccb=vod.$list['data'][$i]['link'];
						echo "<li class='col-md-7 col-sm-4 col-xs-3 ";
						if ($i>=27){echo "hidden-xs";}
						echo "'><div class='stui-vodlist__box stui-vodlist__bg'> <a class='stui-vodlist__thumb lazyload' href='$ccb' title='".$list['data'][$i]['title']."' data-original='".$list['data'][$i]['cover']."'><span class='play hidden-xs'></span>";
						if (!empty($list['data'][$i]['point'])) {echo "<span class='pic-tag pic-tag-h'>".$list['data'][$i]['point']."</span>";}
						echo "<span class='pic-tag pic-tag-b'>".$list['data'][$i]['tag']."</span></a><div class='stui-vodlist__detail active'><h4 class='title text-overflow'><a href='$ccb' title='".$list['data'][$i]['title']."'>".$list['data'][$i]['title']."</a></h4><p class='text text-overflow text-muted hidden-xs'>".$list['data'][$i]['desc']."</p></div></div></li>";
						}?>
				</ul>
			</div>
		</div>
