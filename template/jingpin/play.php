...
<?php if(strpos($zypic[0],'iqiyipic.com') !== false){ echo'<meta name="referrer" content="same-origin">';}?>
...
function run(){
	...
	function adsUp(){
		...
		if ($cat==$dycat) {echo"$mjk$c[0]";}else if($cat==$dscat||$cat==$dmcat){echo "$mjk$b[0]";}else{echo"$mjk$zyvi[1]";}?>" allowfullscreen="true" allowtransparency="true" style="width:100%;border:none"></iframe>');}
//五秒钟后自动收起
var t = setTimeout(adsUp,<?php echo $mkcms_miaoshu*1000;?>);   
				...
				<li><a href="<?php echo $mkcms_domain;?>addfav.php?title=<?php echo $timu; ?>&dizhi=<?php echo 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];?>" class="btn btn-sm btn-default" title="收藏">收藏</a></li>
...
<!-- 播放器-->
			<div class="stui-pannel stui-pannel-bg clearfix">
				...
				echo "<li class='dyli active'><a href='$vod' id='$num' title='$timu'>";
				echo "$dyname</a></li>";$num++;}
